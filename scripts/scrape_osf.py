...
INITIAL_MAX_CONCURRENT_REQUESTS = 5
...
GLOBAL_RATE_LIMIT = 5.0  # max requests per second
...
async def fetch_with_retry(
    ...
    async with semaphore:
        dynamic_delay = REQUEST_DELAY * (1 + recent_rate_limit_rate * 5)
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                logger.warning("Unexpected error fetching %s: %s: %s", osf_id, type(e).__name__, e)
                return (osf_id, None, was_rate_limited)
...
async def process_batch(
    ...
    async def process_id(osf_id: str) -> None:
        ...
        _, response_data, was_rate_limited = await fetch_with_retry(
            session, osf_id, semaphore, rate_limit_tracker, token_bucket, recent_rate_limit_rate
        )

        # Thread-safe update of rate limit tracker
...
async def process_ids_in_batches(ids_file: Path, output_file: Path, successful_ids_file: Path, resume: bool) -> None:
    """Process IDs in batches with batch-level tracking."""
    current_max_concurrent = [INITIAL_MAX_CONCURRENT_REQUESTS]
    rate_limit_tracker: deque = deque(maxlen=RATE_LIMIT_WINDOW)
    token_bucket = TokenBucket(rate=GLOBAL_RATE_LIMIT, capacity=GLOBAL_RATE_LIMIT)
    batch_stats = []
    ...
    if not resume:
        ...
        print("Resume mode: appending to existing output files")
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout, headers=headers) as session:
        for batch_num in range(total_batches):
            ...
            # Compute recent rate limit rate for adaptive behaviour
            recent_rate_limit_rate = (
                sum(rate_limit_tracker) / len(rate_limit_tracker)
            ...
            batch_results = await process_batch(
                ...
                current_max_concurrent, rate_limit_tracker, token_bucket, recent_rate_limit_rate
            )
            ...
            # Inter-batch cooldown when failure rate is high
            failure_rate = batch_results['failed'] / batch_results['total'] if batch_results['total'] > 0 else 0
            if failure_rate > 0.2 and batch_num < total_batches - 1:
                cooldown = 10.0 * (1 + failure_rate)
                print(f"  High failure rate ({failure_rate:.1%}), cooling down for {cooldown:.1f}s...")
...
Examples:
  ...
  python scripts/scrape_osf.py --file data/osf_ids.txt --resume
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Resume a previous run by appending to existing output files instead of overwriting",
    )
    ...
    print(f"Global rate limit: {GLOBAL_RATE_LIMIT} req/s")
    print(f"Adaptive rate limiting: enabled")
    print(f"Resume mode: {args.resume}")
    print()

    start_time = time.time()
    asyncio.run(process_ids_in_batches(args.file, args.output, args.successful_ids, args.resume))
